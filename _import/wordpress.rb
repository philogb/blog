    ...
    def self.process(dbname, user, pass, host = 'localhost', port = 3306)
      ...
      db[QUERY].each do |post|
        ...
        content = post[:post_content] \
                    .gsub("\t", "  ") \
                    .gsub("wp-content", "assets") \
                    .gsub(/<pre name="ocaml" class="ocaml-pre">/, "{% highlight ocaml %}") \
                    .gsub(/<pre class="ocaml-pre">/, "{% highlight ocaml %}") \
                    .gsub(/<pre name="code" class="([a-z]+).+".*>/, "{% highlight \\1 %}") \
                    .gsub("</pre>", "{% endhighlight %}")
